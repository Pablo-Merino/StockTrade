$(function() {
	...
	var data = $('#user_graph').data('history').split(",");

	var margin = {top: 20, right: 80, bottom: 60, left: 90}
  , width = 450 - margin.left - margin.right
  , height = 200 - margin.top - margin.bottom;

	var x = d3.scale.linear().domain([1, data.length]).range([0, width]);
	...
	var graph = d3.select("#user_graph svg").append("svg:g").attr("transform", "translate(" + margin.left + "," + margin.top + ")");
